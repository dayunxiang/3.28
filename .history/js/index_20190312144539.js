$(function () {
    ...
    $('.rotate-btn').on('click',function(){
        $(this).parent().children().css('transform','rotateY(180deg)');
    });

    //canvas
    myCanvas = function (cId,cdata,color,color1,threshold) {
        if(cdata>=50 && cdata<70){
            color = '#ff8602';
            color1 = '#5f3406';
        }else if(cdata>=70){
            color = '#c82723';
    ...
    /* myCanvas('c_2',76,'#13aec0','#06263c',30);
    myCanvas('c_3',76,'#13aec0','#06263c',50); */
    myCanvas('c1',50.53,'#13aec0','#06263c',60);
    ...
    function gongdan () {
        ...
        option = {
            ...
            xAxis: [
                {
                    type: 'category',
                    data: ['无线', '光宽', '动力', '交换', '数据', '传输', 'IPRAN', '业务平台', '核心网'],
                    axisTick: {
                        alignWithLabel: true
                    },
                    axisLabel: {
    ...
    $('.slide_btn').on('click',function(){
        if($(this).hasClass('show')){
            $('.home').animate({'width':0,'borderWidth':0},function(){
                $('.slide_btn').removeClass('show');
                $('#hideTitle').hide();
            });
        }else{
            $('.home').animate({'width':'472px','borderWidth':2});
            $(this).addClass('show');
    ...
    //链接列表
    $('#link_list').on('click','li',function(){
        var $iframe = $('#iframe');
        var $this = $(this);
